...
public static class UTF16 {
	...
	IsHighSurrogate(highSurrogate) && IsLowSurrogate(lowSurrogate)
		? new Rune(Decode((UInt32)highSurrogate, (UInt32)lowSurrogate))
	...
	public static ReadOnlyMemory<Rune> Decode(String? units) => units is not null ? Decode(units.AsMemory()) : ReadOnlyMemory<Rune>.Empty;
	...
	/// <returns>The decoded <see cref="Rune"/>s.</returns>
	public static ReadOnlyMemory<Rune> Decode(params Char[]? units) => units is not null ? Decode(units.AsMemory()) : ReadOnlyMemory<Rune>.Empty;
	...
	/// <returns>The decoded <see cref="Rune"/>s.</returns>
	[CLSCompliant(false)]
	public static ReadOnlyMemory<Rune> Decode(params UInt16[]? units) => units is not null ? Decode(units.AsMemory()) : ReadOnlyMemory<Rune>.Empty;
	...
	public static void Encode(Rune rune, out Char high, out Char low) {
		...
		case 1:
			...
			break;
		case 2:
			Encode(unchecked((UInt16)rune.Value), out high, out low);
			break;
		default:
			high = '\uFFFD';
			low = '\uFFFD';
	...
	/// Assuming both myself and Microsoft don't have any bugs in our code, this should always succeed, because <see cref="Rune"/> uses validating constructors. However there are cases where, inside of the .NET runtime, Rune is created without validation, so, it's possible in theory.
	/// </remarks>
	public static ReadOnlyMemory<Char> Encode(params Rune[]? runes) => runes is not null ? Encode(runes.AsMemory()) : ReadOnlyMemory<Char>.Empty;
	...
	/// </remarks>
	[CLSCompliant(false)]
	public static unsafe ReadOnlyMemory<Char> Encode(Rune* runes, Int32 length) => runes is not null ? Encode(new ReadOnlySpan<Rune>(runes, length)) : ReadOnlyMemory<Char>.Empty;
	...
	private static UInt32 Decode(UInt32 highSurrogate, UInt32 lowSurrogate) => unchecked((highSurrogate << 10) + lowSurrogate - ((0xD800u << 10) + 0xDC00u - (1 << 16)));


	private static void Encode(UInt32 smpScalarValue, out Char highSurrogate, out Char lowSurrogate) {
	...
	private static void Encode(Rune rune, Span<Char> result, ref Int32 index) {
		...
		case 1:
			...
			break;
		case 2:
			Encode((UInt32)rune.Value, out result[index++], out result[index++]);
			break;
		}
	}
	...
	public static class LE {
		...
		/// </summary>
		[SuppressMessage("Performance", "HAA0501:Explicit new array type allocation", Justification = "This has to allocate.")]
		public static ReadOnlyMemory<Byte> BOM { get; } = new Byte[] { 0xFF, 0xFE };
	}
}
